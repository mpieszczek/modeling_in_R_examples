...
library(png)
```
# Odrwracamy kolor w danych testowych:
```{r}
mnist <- dataset_mnist()
...
Teraz sprawdźmy jak taki model poradzi sobie na odwróconych zdjęciach.
```{r}
model_dense %>% evaluate(x_test, y_test)
```
Jak widzimy słabo dokładność wynosi zaledwie 8%. Dla porównania model nadając losowe etykiety powinien osiągnąć 10% na wzgldnie regularnym zbiorze testowym.
W zbiorze testowym mamy w miarę równomierny rozkład liczb.
```{r}
table(mnist$test$y)
```
...
y_test_pred <- y_test_pred %>% as.matrix() %>% as.vector()
...
results
```
Mamy zdcydowaną poprawę w porównaniu z poprzednimi 10%. Dokładność ~50%, to wciąż mało, ale jako że uczyliśmy model na danych o odróconych kolorach, ciekawym jest że sobie z nimi w ogóle poradził.

```{r fig.height=10, fig.width=10}

y_test_pred_conv <- predict(model_conv, test_images) %>% k_argmax()
y_test_pred_conv <- y_test_pred_conv %>% as.matrix() %>% as.vector()
...
for (i in 1:25) {
  plot(as.raster(bledy_x[i, , ], max = 255))
  title(paste("cyfra:", bledy_y[i], "  pred:", y_test_pred_conv[numery_z_bledami][i]))
}
```
...
x[1,,] = readPNG("cyfry_1.png")[,,1]
x[2,,] = readPNG("cyfry_2.png")[,,1]
x[3,,] = readPNG("cyfry_3.png")[,,1]
x[4,,] = readPNG("cyfry_4.png")[,,1]
x[5,,] = readPNG("cyfry_5.png")[,,1]
x[6,,] = readPNG("cyfry_6.png")[,,1]
...
my_y_test_pred_conv <- my_y_test_pred_conv %>% as.matrix() %>% as.vector()
