...
raw_data <- raw_data %>%
  mutate(x = cos(angle) * radius, y = sin(angle) * radius, color_org = ifelse(radius > 2.5, colors[1], colors[2])) %>%
...
workflow_color <- workflow_set(
  ...
  models = list(tree = color_tree, log_reg = color_log_reg, bayes = color_bayes),
  cross = TRUE
)
workflow_color
```
